...
public struct SegmentationInput {
...
public struct PromptSegment: Identifiable, Codable {
    public let id: UUID
    ...
    public init(id: UUID = UUID(), text: String, duration: TimeInterval, order: Int, metadata: [String: String] = [:]) {
        self.id = id
        self.text = text
...
public struct SegmentationMetrics: Codable {
    public let averageDuration: TimeInterval
...
public final class SegmentationModule: ModuleProtocol {
    ...
    public func validate(input: SegmentationInput) -> Bool {
    ...
    private func createSegment(text: String, order: Int, duration: TimeInterval) -> PromptSegment {
        PromptSegment(id: UUID(), text: text.trimmingCharacters(in: .whitespacesAndNewlines), duration: duration, order: order, metadata: [:])
    }
    ...
    private func detectTransitionType(_ segment: PromptSegment, previousSegment: PromptSegment?) -> TransitionType {
        ...
        if segment.text.contains("\"") { return .dialogue }
        return .cut
    }
...
private enum TransitionType: String {
    case cut = "Cut"
    case fade = "Fade"
