...
public struct ContinuityModule: PipelineModule {
    ...
    public func validate(input: ContinuityInput) -> [String] {
        ...
        if input.segments.isEmpty {
            warnings.append("No segments provided for continuity validation")
        }
        
        if input.segments.count == 1 {
            warnings.append("Only one segment - continuity validation limited")
        }
    ...
    private func generateContinuityAnchors(from segments: [PromptSegment]) -> [ContinuityAnchor] {
        ...
        for (index, segment) in segments.enumerated() {
            ...
            for character in state.characters {
                ...
                characterMap[character]?.insert(segment.text)
            }
        }
...
private struct SceneState: Sendable, Codable {
    let id: Int
    let location: String
